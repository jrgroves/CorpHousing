  ...
  ifelse(i<2018,
    ...
    fown_dat <- parcel %>%
      ...
      mutate(OWN_ADD = str_replace_all(OWN_ADD, "P O ", "PO "),
             ...
             OWNER_NAME = str_replace_all(OWNER_NAME, "J/T", "JOINT TRUST"),
      ...
      mutate(key = Corporate+Partner+Trust+Bank+Instit+hoa+muni,
             private = case_when(key==0 ~ 1,
                                 TRUE ~ private),
             year = i) %>%
      arrange(PROP_ADD, .keep_all = TRUE) 
    
    assign(paste0("fown_dat",i), fown_dat)
