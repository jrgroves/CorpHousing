...
#Load Data
  ...
  mod.1a <-lm_robust(myModel, cluster = GEOID, data = rs_core1.4)
  ...
  myModel <- as.formula(paste(depVar,indepVars,sep = ' ~ '))
  mod.1b <-lm_robust(myModel, cluster = GEOID, data = rs_core1.4)
  ...
  indepVars = paste("nb_corporate", "nb_private", "nb_legal", "nb_other", time.dum,
                    census, sep = "+")
  myModel <- as.formula(paste(depVar,indepVars,sep = ' ~ '))
  mod.2a <-lm_robust(myModel, cluster = GEOID, data = rs_core1.4) 
  ...
  indepVars = paste("nb_corporate", "nb_private", "nb_legal", "nb_other", "trans.ten",
                    time.dum, census, sep = "+")
  myModel <- as.formula(paste(depVar,indepVars,sep = ' ~ '))
  mod.2b <-lm_robust(myModel, cluster = GEOID, data = rs_core1.4)
  ...
  mod.2c <-lm_robust(myModel, cluster = GEOID, data = subset(rs_core1.4, trans.end.own == "Private"))
  ...
  tab2 <- tbl_merge(list(tab2a, tab2b,tab2c), tab_spanner = c("**Model One**", "**Model Two**",
    ...
    as_flex_table() %>%
    add_footer_lines("Standard Errors Clustered as Census Tract") %>%
    add_header_lines("Table Four: One Quarter Mile Radius") %>%
    bold(part = "header", i = 1) %>%
    align(align = "center", part = "header") %>%
    fontsize(part = "header", size = 14) %>%
    fontsize(part = "body", size = 11) %>%
