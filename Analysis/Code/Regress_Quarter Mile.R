...
#Define Variables 
  ...
  mod.1 <-lm_robust(myModel, cluster = GEOID, data = rs_core1.4)
  ...
  myModel <- as.formula(paste(depVar,indepVars,sep = ' ~ '))
  mod.2 <-lm_robust(myModel, cluster = GEOID, data = rs_core1.4)
...
#Main Model 
  ...
  myModel <- as.formula(paste(depVar,indepVars,sep = ' ~ '))
  mod.3 <-lm_robust(myModel, cluster = GEOID, data = rs_core1.4) 
  ...
  mod.3a <-lm_robust(myModel, cluster = GEOID, data = subset(rs_core1.4, trans.end.ten == "Owner"))
  mod.3b <-lm_robust(myModel, cluster = GEOID, data = subset(rs_core1.4, trans.end.ten == "Not Owner"))
  ...
  tab2 <- tbl_merge(list(tab2a, tab2b,tab2c), tab_spanner = c("**Full Sample**", "**Owners**", "**Not Onwers**")) %>%
    as_flex_table() %>%
    add_footer_lines("Standard Errors Clustered as Census Tract") %>%
    add_header_lines("Table Five: One-Quarter Mile Neighborhood") %>%
    align(., align = "center", part = "header") %>%
    bold(part = "header")
  
  mod.3c <-lm_robust(myModel, cluster = GEOID, data = subset(rs_core1.4, trans.end.own == "Private"))
  mod.3d <-lm_robust(myModel, cluster = GEOID, data = subset(rs_core1.4, trans.end.own == "Not Private"))
  ...
  tab3 <- tbl_merge(list(tab2a, tab2c,tab2d), tab_spanner = c("**Full Sample**", "**Private**", "**Not Private**")) %>%
    as_flex_table() %>%
    add_footer_lines("Standard Errors Clustered as Census Tract") %>%
    add_header_lines("Table Six: One-Quarter Mile Neighborhood") %>%
    align(., align = "center", part = "header") %>%
    bold(part = "header")
  
  indepVars = paste("nb_nonzip","nb_owner",indepVars, sep = "+")
  myModel <- as.formula(paste(depVar,indepVars,sep = ' ~ '))
  mod.4 <-lm_robust(myModel, cluster = GEOID, data = rs_core1.4)
  ...
  mod.4a <-lm_robust(myModel, cluster = GEOID, data = subset(rs_core1.4, trans.end.ten == "Owner"))
  mod.4b <-lm_robust(myModel, cluster = GEOID, data = subset(rs_core1.4, trans.end.ten == "Not Owner"))
  ...
  tab4 <- tbl_merge(list(tab4a, tab4b,tab4c), tab_spanner = c("**Full Sample**", "**Owners**", "**Not Owners**")) %>%
    as_flex_table() %>%
    add_footer_lines("Standard Errors Clustered as Census Tract") %>%
    add_header_lines("Table Seven: One-Quarter Mile Neighborhood") %>%
    align(., align = "center", part = "header") %>%
    bold(part = "header")
  
  
  save_as_docx(tab1, tab2, tab3, tab4, path = "./Analysis/Output/Quarter Tables.docx")
