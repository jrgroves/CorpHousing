...
#Difference between sales of properties.
    rs_core2 <- rs_core %>%
      ...
      group_by(parid) %>%
        mutate(
          pre_type = case_when(pre_corporate == 1 ~ "Corporate",
                               pre_private == 1 ~ "Private",
                               TRUE ~ "Other"),
          post_type = case_when(post_corporate == 1 ~ "Corporate",
                                post_private == 1 ~ "Private",
                                TRUE ~ "Other"),
          pre_TT = paste(pre_type, pre_tenure, sep = " "),
          post_TT = paste(post_type, post_tenure, sep = " "),
          ...
          from.ten = lag(pre_tenure),
          to.ten = post_tenure,
          from.typ = lag(pre_type),
          to.typ = post_tenure,
          from.tt = lag(pre_TT),
          to.tt = post_TT,
      ...
      filter(!is.na(d_adj_price),
             ...
             d_nb_corporate > -.40,
             d_saleyr != 0) %>%
      select(d_ln_price, d_adj_price, from.ten, to.ten, from.typ, to.typ, from.tt, to.tt, d_nb_corporate,
             starts_with("per_"), starts_with("nb_"), N2N, N2O, O2N, O2O, P2P, P2C, C2P, C2C, ten1, 
             trans.own, starts_with("d_")) %>%
      mutate(from.to = paste(from.tt, to.tt, sep=" "))
    ...
    c<-filter(rs_core2, to.tt=="Private OWNER")
    mod.1a <-lm(myModel, data=c)
    
    c<-filter(rs_core2, to.tt=="Private NONOWNER")
    mod.1b <-lm(myModel, data=c)
    
    c<-filter(rs_core2, to.tt=="Corporate NONOWNER")
    mod.1c <-lm(myModel, data=c)
